...
test('release runtime uses the neutral Beta v43 branding contract', () => {
  ...
  const versionedAssets = [
    ...html.matchAll(/<link\b(?=[^>]*\brel=["']stylesheet["'])[^>]*\bhref=["'](\.\/[^"']+)\?v=([^"']+)["'][^>]*>/g),
    ...html.matchAll(/<script\b[^>]*\bsrc=["'](\.\/[^"']+)\?v=([^"']+)["'][^>]*>/g),
  ];
  assert.ok(versionedAssets.length > 0);
  for (const [, assetPath, assetSuffix] of versionedAssets) {
    assert.match(assetPath, /^\.\//);
    assert.equal(assetSuffix, EXPECTED_ASSET_SUFFIX);
  }
  ...
  assert.match(stylesheet, new RegExp(`^/\\* Studio Workspace v${EXPECTED_ASSET_SUFFIX.replaceAll('.', '\\.')} \\*/`));
  assert.match(adapter, new RegExp(`^// Studio Workspace v${EXPECTED_ASSET_SUFFIX.replaceAll('.', '\\.')}`));
